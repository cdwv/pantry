...
module.exports = function(options) {
    config = {
        ...
        shelfLife: options.shelfLife || 60,
        maxLife: options.maxLife || 300,
        caseSensitive: options.caseSensitive || true,
        ignoreCacheControl: options.ignoreCacheControl || false,
        cacheBuster: options.cacheBuster,
        verbosity: options.verbosity || process.env.PANTRY_VERBOSITY || getLogLevel(),
        xmlOptions: {
            explicitRoot: false
        }
    };
    var log = config.logger || new(winston.Logger)({
        transports: [
    ...
    Pantry.storage = options.storage;
    Pantry.backup = new MemoryStorage(config, config.verbosity);
